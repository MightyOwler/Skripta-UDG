...
vloženimi v prostor $\R ^3$. 

\begin{definicija}
\label{def_gladka_vložena_ploskev} Množica
$X \subseteq \R ^3$ je gladka vložena ploskev, če
za vsako točko $m \in  X$ obstaja krogla $W \ni m \subseteq
\R ^3$ ter gladka funkcija $f : W \to \R $,
za katero velja 
...
\subseteq \R ^2$  in gladka funkcija $g: V \to \R $,
...
produkt na $\mathcal{V} = \R ^n$ in naj bo $\left\{
...
zapisi linearnih preslikav iz $\R ^n \to
\R ^n$, lahko pa so tudi koordinatni zapisi
skalarnih produktov. To je odvisno od tega, kako se
matrike transformirajo pri prehodu v različno bazo.

Naj bo $\mathcal{L}: \R ^n \to  \R ^n$ linearna, $L_e$ matrika
...
$P^{T}A_fP = A_e$ oziroma v primeru ortogonalnosti
...
\label{def_rang_preslikave}
 Naj bo preslikava $F: W \subseteq  \R ^n \to  \R ^m$ odvedljiva.
...
\begin{definicija} 
	...
	Naj bo $X \subseteq \R ^3$ vložena ploskev in točka $m
...
\to  X \subseteq  \R ^3 \text{ krivulja, } \gamma\left(
...
\end{trditev} 
{\em Dokaz:\/} Naj bo $r : V
\subseteq  \R ^2 \to X \subseteq \R ^3$ neka regularna
parametrizacija ploskve $X$ (to pomeni, da mora biti rang preslikave
$r$ maksimalen, torej konstantno enak 2) v okolici točke  $m \in  X$.  Označimo za vsak $\left( u, v \right) \in  V \subseteq \R ^2$ \begin{equation*} r\left( u,v \right) = 
...
\varepsilon \right) \to X \subseteq \R ^2,$ $\gamma \left( 0
...
$\R ^2,$ obstajata funkciji $u\left( t \right), v\left( t
...
vektor $w \in \R ^2$ in naj bo $v = \left( D_{(u_0, v_0)} r \right) w$. Potrebujemo krivuljo $\gamma(t) : (-\varepsilon, \varepsilon) \to  X,
...
\varepsilon)\to  X \subseteq \R ^3$, hkrati pa tudi $\gamma(0) = r(\beta(0) )= r(u_0, v_0) = m$, je po definiciji tangentnega prostora $\dot{\gamma}(t) \in T_mX$. Torej velja,
...
\label{def_metrika_na_ploskvi}
 Metrika na ploskvi $X \subseteq  \R ^3$, opremljeni s
 parametrizacijo $r: V \subseteq  \R ^2 \to  X \subseteq
 \R ^3$, je preslikava \begin{align*}
 	g: X &\longrightarrow M_2(\R ), \\
 	m &\longmapsto 
	\begin{pmatrix}
		g_{11}(m) & g_{12}(m) \\\
		g_{21}(m) & g_{22}(m) \\
...
funkcije $u,v : [a,b] \to \R $, $\beta(t) = (u(t), v(t))$,
...
Naj bo $X \subseteq \R ^3$ ploskev ter $r: V \subseteq \R ^2 \to X$ njena parametrizacija. Naj bo $\gamma: [a,b] \to  X$ krivulja, ki ji pripadata funkciji $u, v: [a,b] \to  \R $, tako da velja \begin{equation*}
\gamma(t) = r(u(t), v(t)).
\end{equation*}  
...
g_{22}$. Naj bo sedaj ambientni prostor $\R ^3$ opremljen s
...
\label{def_prva_fundamentalna_forma}
 Metrika na $X \subseteq  \R ^2$, ki je glede na parametrizacijo $r: V \to  X
\subseteq  \R ^3$ podana z matrično funkcijo \begin{align*}
 	g_f: V &\longrightarrow M_2(\R ) \\
	(u,v) &\longmapsto 
\begin{pmatrix}
	E(u,v) & F(u,v) \\
	F(u,v) & G(u,v) \\
...
\label{def_izometrija}
 ...
 \R ^3$ je izometrija, če za vsako krivuljo $\gamma: \left[ a,b
...
izometrijo $f: X \to  \tilde{X}.$ Naj bo $\gamma(t): (0, \varepsilon) \to X$ krivulja s predpisom $\gamma(t) = r(\beta_1(t)) =
...
\begin{primer}
 ...
 \begin{equation*} S = \left\{ (x,y,z) \in \R ^3 \middle|\, x^2 + y^2  = z^2 \right\}.\end{equation*}
...
Na tej točki se pojavi naravno vprašanje: ali znamo poiskati vse ploskve v $\R ^3$, ki so izomertične ravnini?
...
\begin{definicija}
  \label{def_tangentno_premonsna_ploskev}
   Naj bo $\gamma: [a,b] \to  \R ^3$ prostorska krivulja, parametriziarana z naravnim parametrom. Tangentna premonosna ploskev,
   podana s krivuljo $\gamma$, je del prostora $\R ^3$, ki ga opiše tangenta na $\gamma(u)$ na intervalu $u \in  [a,b]$. Parametriziramo jo lahko s predpisom \begin{equation*}
...
\end{izrek}
Da je plašč stožca izometričen kosu ravnine, smo že videli v primeru (za valj je to še bolj očitno). V nadaljevanju bomo pokazali, da enako velja tudi za tangentno premonosne ploskve.
Dejstvo, da so to res edine tri možnosti, bomo privzeli brez dokaza.
...
\label{trd_obstoj_krivulje_glede_na_funkcijo_fleksijske_ukrivljenosti}
 Naj bo podana poljubna zvezna funkcija $\tilde{\varkappa}: [\alpha, \beta] \to  \R $. Potem obstaja naravno parametrizirana ravninska krivulja $\gamma: [\alpha, \beta] \to  \R ^2$, katere fleksijska ukrivljenost v točki
...
{\em Dokaz:\/}
 ...
 torej bo za neko funkcijo $k : [\alpha, \beta]  \to \R $ veljalo 
...
\label{def_ploscina_plosvke}
  Ploščina ploskve $X$, regularno parametrizirane z $r: V \subseteq  \R ^2 \to  X \subseteq  \R ^3$ je podana z 
...
\label{def_prehodna_preslikava_med_parametrizacijama}
 Naj bosta $r: V_1 \subseteq  \R ^2 \to  X \subseteq  \R ^3$ in $\tilde{r}: V_2 \subseteq  \R ^2 \to  X \subseteq  \R ^3$ različni regularni parametrizaciji ploskve $X$.
...
{\em Dokaz:\/}
 ...
 \begin{align*}
     ...
     \end{pmatrix}}   \, du \, dv \\
      ...
      &= \int_{V_1} \sqrt{\det \begin{pmatrix}
        \tilde{E} & \tilde{F} \\
        \tilde{F} & \tilde{G}
      \end{pmatrix}} \left| \det \left( \operatorname{Jac}(g) \right) \right|  \, du \, dv \\ 
     \text{(uvedba novih spremenljivk)} &= \int_{V_2} \sqrt{\det \begin{pmatrix}
      \tilde{E} & \tilde{F} \\
      \tilde{F} & \tilde{G}
...
Recimo, da imamo podano $V \subseteq  \R ^2$ in matrično funkcijo \begin{align*}
  M: V &\longrightarrow M_2(\R )_\text{simetrične, pozitivno definitne}  \\
...
Naj bo $X \subseteq \R ^3$ ploskev s parametrizacijo $r: V \subseteq \R ^2 \to  X$.
...
\label{def_druga_fundamentalna_forma_ploskve}
 Naj bo $X \subseteq \R ^3$ ploskev s parametrizacijo $r: V \subseteq \R ^2 \to  X$. Druga fundamentalna forma $X$ v točki $m = r(u,v)$ je podana z matriko 
 \begin{equation*} \begin{pmatrix}
 L & M \\
 M & N
 \end{pmatrix}_{(u,v)} = \begin{pmatrix}
 ...
 M(u,v) & N(u,v)
 \end{pmatrix}, \end{equation*}
 kjer so $L, M, N: V \to  \R $ funkcije s predpisi 
 \begin{align*}
...
\begin{opomba}
    Za drugo fundamentalno formo je res nujen skalarni produkt v $\R ^3$ (za razliko od prve fundamentalne forme, ki v vsaki točki določa drug skalarni produkt). 
\end{opomba}
...
V nadaljevanju bomo razdelili vektor pospeška na geodetsko in normalno komponento, torej 
...
Naj bo $X \subseteq \R ^3$ ploskev s parametrizacijo $r: V \subseteq \R ^2 \to  X$ in naj bo $\gamma: [a,b] \to X$ gladka krivulja. 
Definirajmo normalo na krivuljo $\gamma$ kot preslikavo $n_{\gamma}: [a,b] \to  TX^{\perp}$ s predpisom
...
\label{def_razsiritev_normalne_ukrivljenosti}
    ...
    Potem obstaja funkcija \begin{align*}
       \varkappa_n : T_mX &\longrightarrow \R , \\
...
\begin{opomba}
 ...
 na enotski krožnici v ravnini $T_mX$. V $\R ^3$ je to res običajna krožnica glede na
...
\label{def_glavni_ukrivljenosti_ploskve}
 Ekstrema funkcije $\varkappa_n: T_mX \to  \R $, skrčene na enotsko sfero $E \xi^2 + 2F \xi \eta + G \eta^2  = 1$, imenujemo glavni ukrivljenosti ploskve $X$ v točki $m$. 
...
\noindent
{\em Dokaz:\/}
 Ekstrema $\lambda_1$, $\lambda_2$ sta vezana ekstrema za funkcijo $\varkappa_n: T_mX \to \R $.
 Zaradi homeomorfizma $T_mX \approx \R ^2$ lahko identificiramo elemente $T_mX$ s pari $(\xi, \eta)$. Naša krožnica je
   ...
   \end{equation*}  
    ...
    \end{pmatrix} \right) = 0.
     ...
     \begin{equation*}
        ...
        \end{pmatrix}.
     \end{equation*}  
    Če vzamemo enotska vektorja $(\xi_1, \eta_1)^{T}$, $(\xi_2, \eta_2)^{T}$ (ki sta enotska glede na 1. fundamentalno formo v točki $m$), in množimo $i$-to zgornjo enačbo z leve z $(\xi_i, \eta_i)$, 
    dobimo 
    \begin{equation*}
        \underbrace{\begin{pmatrix}
            \xi_i & \eta_i
...
Za Gaussovo ukrivljenost $\kappa: X \to  \R $ velja zveza \begin{equation*}
...
Glavni smeri $(\xi_1, \eta_1)_{(m)}^{T}$ in $(\xi_2, \eta_2)_{(m)}^{T}$ sta si pravokotni v $T_m X$. 
\end{izrek}
\noindent
{\em Dokaz:\/}
...
je dovolj dokazati sebi adjungiranost preslikave $C$. Torej za vsaka vektorja $\vec{v}, \vec{w} \in  T_m X$ dokazujemo \begin{equation*}
...
\subsubsection{Interpretacija Gaussove ukrivljenosti}

Intuitivno je ukrivljenost krivulje hitrost spreminjanja tangente. Hkrati pa velja, da čim bolj je krivulja ukrivljena, tem hitreje
...
Naj bo $X \subseteq  \R ^3$ ploskev. Preslikava, ki točki priredi enotsko normalo, \begin{align*}
...
\end{equation*}  
kjer je $A(U)$ ploščina okolice $U \subseteq X$, $A(\tilde{n}_{*}(U))$ pa je ploščina slike \begin{equation*}
...
\lim_{V \to m} \frac{A(\tilde{n}(U))}{A(U)} = \frac{(LN - M^2)(m)}{(EG - F^2)(m)} = \kappa(m).
\end{equation*}  
...
Naj bo $X$ poljubna ploskev. Tedaj obstaja za $X$ parametrizacija $s : V \to  X$ ter funkciji $\lambda, \mu : V \to  \R $, za kateri velja
\begin{align*}
  ...
  \begin{pmatrix}
    ...
    \end{pmatrix} = \begin{pmatrix}
      ...
      \end{pmatrix} = \begin{pmatrix}
        ...
        \end{pmatrix}.
\end{equation*}  
Iz analize 3 vemo, da lahko v takšni situaciji najdemo integrirajoča množitelja $\lambda, \mu: V \to \R $, da bo veljalo \begin{align*}
...
Naj bo $\gamma: [a,b] \to \R ^3$ krivulja in $\tilde{a}: \R ^3 \to \R ^3$ vektorsko polje. 
...
razvili vektorja $n_u$ in $n_v$. Pri tem se zavedajmo, da je $e_1 = \frac{r_u}{\lvert\lvert r_u \rvert\rvert }$ in $e_2 = \frac{r_v}{\lvert\lvert r_v \rvert\rvert }$.
...
$\gamma(u) = r(u, v_0)$. Naša ploskev je premonosna, lahko jo parametriziramo z \begin{equation*}
