...
\begin{equation*} \ddot{\gamma} = \ddot{\gamma}_g + \ddot{\gamma}_n.\end{equation*}

Zato najprej definirajmo geodetsko in normalno ukrivljenost.
...
\begin{equation*} n_{\gamma}(t) = \frac{r_u(u(t), v(t)) \times  r_v(u(t), v(t))}{\lvert\lvert r_u(u(t), v(t)) \rvert\rvert }.\end{equation*}
Potem definiramo normalno in geodetsko ukrivljenost kot 
\begin{align*}
...
Zdaj si poglejmo, kako se izraža normalna ukrivljenost s pomočjo druge fundamentalne forme.
...
\label{def_razsiritev_normalne_ukrivljenosti}
    Naj bo $X \subseteq \mathbb{R}^3$ ploskev s parametrizacijo $: V \subseteq \mathbb{R}^2 \to  X$
    ...
    Potem obstaja funkcija \begin{align*}
       \kappa_n : T_mX &\longrightarrow \mathbb{R} \\
        \kappa_n(\xi r_u(u_0, v_0) + \eta r_v(u_0, v_0)) =: \kappa_n(\xi, \eta) &\longmapsto 
        \begin{pmatrix}
          \xi  & \eta
        \end{pmatrix}
        \begin{pmatrix}
...
\begin{opomba}
 Če je $(\xi, \eta) = (\dot{u}, \dot{v})$ in $\gamma$ naravno parametriziana krivulja, potem par $(\xi, \eta)$ leži
 na enotski krožnici v ravnini $T_mX$. V $\mathbb{R}^3$ je to res običajna krožnica glede na
 evklidkski skalarni produkt, v koordinatah $(\xi, \eta)$ pa je 
...
\label{def_glavni_ukrivljenosti_ploskve}
 ...
 Označimo ju s $\kappa_1$, $\kappa_2$. 
\end{definicija}

\begin{opomba}
 Ali minimum označimo $\kappa_1$ ali $\kappa_2$, je stvar dogovora. Ekstrema obstajata, ker je krožnica kompakt, torej ima zvezna funkcija $\kappa_n$ na njej minimum in maksimum.
\end{opomba}
...
\label{def_povprecna_ukrivljenost}
    Povprečna ukrivljenst ploskve $X$ v točki $m$ je podana s formulo 
 ...
 \end{equation*}  
   pri vezi \begin{equation*}
     ...
     \begin{pmatrix}
       \xi \\
       \eta 
     \end{pmatrix} = E \xi^2 + 2F \xi \eta + G \eta^2.
   \end{equation*}  
    ...
    \end{equation*}  
    Da bo skalar $\lambda$ ekstrem, mora veljati zveza \begin{equation*}
      \operatorname{grad}(\kappa_n(\xi, \eta) - \lambda g(\xi, \eta)) = 0.
    \end{equation*}  
    Zaradi linearnosti gradienta je to ekvivalentno zvezi \begin{equation*}
        \operatorname{grad}(\kappa_n(\xi, \eta)) - \lambda \operatorname{grad}(g(\xi, \eta)).
    \end{equation*}
    Pri tem dobimo \begin{align*}
    ...
    \end{pmatrix} \right) = 0.
     \end{equation*}  
     Denimo, da imamo neki rešitvi $\lambda_1$, $\lambda_2$. Potem obstajata vektorja $(\xi_1, \eta_1)^{T}$, $(\xi_2, \eta_2)^{T}$, da je   
     \begin{equation*}
        \left(
            \begin{pmatrix}
          L & M \\
        ...
        \end{pmatrix}.
     \end{equation*}  
    Če vzamemo enotska vektorja $(\xi_1, \eta_1)^{T}$, $(\xi_2, \eta_2)^{T}$, in množimo zgornjo enačbo z leve z $(\xi_i, \eta_i)$, 
    dobimo 
    \begin{equation*}
        \underbrace{\begin{pmatrix}
            \xi_i & \eta_i
        ...
        \end{pmatrix}.
     \end{equation*}
     Desni člen pride $1$, ker smo izbrali enotska vektorja. Preostaneta nam torej enačbi \begin{equation*}
       \kappa_n(\xi_i, \eta_i) = \lambda_i.
     \end{equation*}  
     Točki $(\xi_i, \eta_i)$ sta vezana ekstrema funkcije $\kappa_n$ pri vezi $g = 1$. Torej sta to ničli
     \begin{equation*}
        \det \left( 
         \begin{pmatrix}
       L & M \\
