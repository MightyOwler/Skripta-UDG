...
Iz dokaza prejšnjega izreka je razvidno, da sta $\kappa_1$ in $\kappa_2$ lastni vrednosti martrike \begin{equation*}
  ...
  \end{pmatrix},
\end{equation*}  
  glavni smeri $(\xi_1, \eta_1)^{T}$, $(\xi_2, \eta_2)^{T}$ pa sta njuna lastna vektorja.
...
\noident
{\em Dokaz:\/}
Po definiciji je $\kappa(m) = \kappa_1(m) \kappa_2(m)$. Razsmilili smo že, da sta glavni ukrivljenosti
...
To sta ničli kvadratne enačbe \begin{equation*}
...
\kappa = \kappa_1 \kappa_2 = \frac{c}{a} = \frac{1}{2} \frac{(LF - 2MF + NE)(m)}{(EG - F^2)(m)}.
\end{equation*}  
\end{opomba}
...
je dovolj, da dokažemo sebi adjungiranost preslikave $C$. Torej za vsaka vektroja $\vec{v}, \vec{w} \in  T_m X$ dokazujemo \begin{equation*}
...
\end{align*}
To pomeni, da mora veljati $AC^{*} = C^{T}A$, iz česar sledi\begin{equation*}
C^{*} = A^{-1}C^{T}A = A^{-1}(A^{-1}B)^{T}A = A^{-1}B^{T}A^{-T}A = A^{-1}B A^{-1} A = A^{-1} B = C.
\end{equation*}  
\qed
...
Naj to torej točka $m \in  X$, ter $U$ njena okolica $m \in  U \subseteq X$, difeomorfna disku. Poglejmo si, kaj se dogaja z normalo po točkah $U$.
...
U \subseteq X \to \tilde{n}_{*}(U) \subseteq S^2. 
\end{equation*}  
Množica $\tilde{n}_{*}(U)$ je podmnožica enotske sfere $S^2$.

Predpostavimo, da je preslikava \begin{center}
