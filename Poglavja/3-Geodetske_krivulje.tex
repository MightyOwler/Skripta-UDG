...
\mathcal{L}(\gamma) = \int_{a}^{b} \sqrt{E(u(t), v(t))\dot{u}^2 + 2F(u(t), v(t)) \dot{u} \dot{v} + G(u(t), v(t)) \dot{v}^2}  \, dt 
\end{equation*}  
...
\end{opomba}
Ekstreme funkcionala $\mathcal{L}: \mathcal{V} \to  \R $ bomo iskali tako, kot iščemo ekstreme
funkcij ene ali dveh spremenljivk. Kandidati za ekstreme splošne funkcije $F: \Omega \subseteq \R ^n \to  \R $ so stacionarne točke $F$, torej
točke $x_0 \in \R ^n$, da je $D_{x_0}F = 0$. Vemo pa, da bo $D_{x_0}F = 0$ natanko tedaj, ko za vsak dovolj majhen $\varepsilon > 0$ in za vsako krivuljo
...
\end{equation*}  
% To je direktna posledica dejstva, da je lahko vektor $\dot{\beta}(0) \in  \R ^n$ poljuben.  TODO poglej, kaj je s to izjavo
...
Na tej točki velja omeniti, da v primeru podane variacije $\Gamma(t,s) = r(u(t,s), v(t,s))$ tangento podaja predpis \begin{equation*}
...
\end{equation*}  
in enako za člen $R^{-\frac{1}{2}} (F\dot{u} + G\dot{v}) v_s(t,s)$. Zdaj le še povzemimo vse skupaj.
Prišli smo do tega, da je krivulja $\gamma(t)$ stacionarna točka funkcionala $\mathcal{L}: \mathcal{V} \to  \R $, če je za vsak par $(t,s)$\begin{equation*}
...
Naj bosta $P, Q: [a,b] \to  \R $ zvezni funkciji in naj bo $\Gamma(t, s) = r(u(t,s), v(t,s)) : [a,b] \times (-\varepsilon, \varepsilon) \to \R ^2$ variacija, torej $u(t, 0) = u(t), v(t, 0) = v(t), u(a,s) = u(a), v(a,s) = v(a), u(b,s) = u(b)$ in $v(b,s) = v(b)$. Potem velja enakost \begin{equation*}
...
\end{align*} 
S tem smo zaključili dokaz za $P$, za $Q$ sledi simetrično.
\qed
...
Po znanem postopku poračunamo prvo fundamentalno formo in dobimo rezultat \begin{equation*}
    ...
    \end{pmatrix}.
\end{equation*}
Ni težko opaziti, da je ta fundamentalna forma zelo podobna formi sfere. V resnici nam zgornji predpis podaja vložitev hiperbolične ravnine v prostor $\R ^3$. 
\end{primer}
...
Poiskali bomo način, kako izraziti $\kappa$ z $E, F, G$ in njihovimi odvodi. Naj bo $r: V \subseteq \R ^2 \to  X \subseteq  \R ^3$ parametrizacija ploskve $X$. Izberimo par ortonormiranih
vektorskih polj $e_1, e_2$ na $X$: \begin{equation*}
e_i(u,v): V \subseteq \R ^2  \to \bigsqcup_{(u,v) \in  V} T_{r(u,v)}X.
\end{equation*}  
...
\end{equation*}
Naj bo $n(u,v) \in (T_{r(u,v)}X)^{\perp}$ enotska normala v točki $r(u,v)$ podana s predpisom $n(u,v) = e_1(u,v) \times  e_2(u,v)$. V vsaki točki je $r(u,v) = m \in  X$ je trojica vektorjev $\left\{ e_1(u,v), e_2(u,v), n(u,v)\right\}$ ortonormirana baza prostora $\R ^3$. 
...
\end{align*}
Pri tem so $\alpha_i, \lambda_i, \mu_i: V \to \R $ funkcije, odvisne od $(u,v)$. Vektor $(e_1)_u$ (in na enak način ostale) razvijemo po bazi: \begin{equation*}
...
V primeru $\kappa = - \frac{1}{a^2}$ dobimo 
\begin{equation*}
