...
Zdaj uporabimo Greenovo formulo, da dobimo \begin{align*}
    I &= \int_{S} \langle e_1, (e_2)_v \rangle_u - \langle e_1, (e_2)_u \rangle_v   \, dS  \\
     ...
     &= \int_{S} \frac{LN - M^2}{\sqrt{EG - F^2} }  \, dS. \,\,\,(\text{sklep iz dokaza Theorema egregium})
\end{align*}
...
Izrazimo $I$ še na drug način. Naj bo $\theta(s)$ kot med $\gamma(s)$ in $e_1(s)$. Ker je $\left\{ e_1(\gamma(s)), e_2(\gamma(s))\right\}$ ortonormirana baza v tangentnem prostoru
...
\sum_{i=1}^{n} \alpha_i = (n-2) \pi + \int_{\gamma} \varkappa_g  \, ds + \int_{R} K \, dA.  
\end{equation*}  
\end{izrek}
...
Torej bo \begin{align*}
    ...
    &\implies \sum_{i = 1}^{n} \alpha_i = (n - 2)\pi + \int_{\gamma} \varkappa_g \, ds + \int_{R} K \, dA.
\end{align*}  
\qed
\begin{posledica}
    \label{psl_posledica_globalnega_GB_izreka}
    Če je $R$ mnogokotnik, omejen z  geodetkami, velja \begin{equation*}
        \sum_{i=1}^{n} \alpha_i = (n-2) \pi  + \int_{R} K \, dA. 
    \end{equation*}  
    \end{posledica}
...
Pokažimo, da je definicija večkratnosti dobra. \begin{enumerate}
    ...
    \end{equation*}  
    kjer je $\varphi(s)$ kot med $\eta(s)$ in $\gamma(s)$.
...
\int_{Y} \kappa  \, dA = \sum_{k = 1}^{n} \int_{\partial R_i} \langle e_1, \dot{e}_2 \rangle   \, ds.  
\end{equation*}
...
Pri tem je $\phi(m)$ kot med $e_1(m)$ in $\dot{\gamma}(m)$ za vsak $m \in  \gamma_i$. Če te formule povežemo, dobimo \begin{align*}
    \int_{Y} \kappa  \, dA  + \sum_{k = 1}^{n} \int_{R_i} \kappa \, dA  &= \sum_{i = 1}^{n} \int_{\gamma_i} \langle e_1, \dot{e}_2 \rangle  - \langle f_1^{i}, \dot{f}_2^{i} \rangle   \, ds.    \\
...
\tilde{f}(u_0 +h, v' + k) = \begin{pmatrix}
    \tilde{h} & \tilde{k} 
  \end{pmatrix}
  \begin{pmatrix}
    \lambda_1 &  \\
...
\end{equation*}  
V primeru pozitivno ali negativno definitne Hessejeve matrike ($\lambda_1 \lambda_2  > 0$) ta enačba predstavlja elipso. Pokažimo, da je gradient vedno pravokoten na nivojnice. Naj bo $\beta(t)$ nivojnica $f$, torej $f(\beta(t)) = \text{kons.}$\begin{equation*}
